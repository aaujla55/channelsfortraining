...
local DB_CONNECT_TIMEOUT = 20  -- number of seconds before the DB connection times out (ODBC default = 300 seconds)
...
function method.info(Self, T)
   local metaTable = ReConnect(Self)
   local Success, Result = pcall(metaTable.Conn.info, metaTable.Conn, T)
   if not Success then error(Result, 2) end
   return Result
end
